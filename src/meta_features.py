...
from .config import GRUPOS_MFE


def limpiar_nombre_meta_feature(nombre: object) -> str:
    """Normalize a PyMFE feature name for pandas/sklearn usage."""
    return (
        str(nombre)
...
def convertir_a_float(valor: object) -> float:
    """Return value as float, or NaN if conversion is not possible."""
    try:
        if pd.isna(valor):
            return np.nan
        return float(valor)
...
def _cat_cols(df: pd.DataFrame) -> list[int]:
...
def _extraer_bloque(
    ...
    X_fit_df: pd.DataFrame | None = None,
    y_fit: pd.Series | np.ndarray | None = None,
) -> dict[str, float]:
    """Extract one PyMFE feature block with the notebook's conversion rules."""
    ...
    mfe.fit(X_fit_df.to_numpy(), y_fit.to_numpy(), cat_cols=_cat_cols(X_fit_df))
...
def extraer_meta_features_pymfe(X_df: pd.DataFrame, y: pd.Series | np.ndarray) -> dict[str, float]:
    """Extract the exact meta-feature set used in the final notebook."""
    X_df = X_df.copy()
    ...
    meta["class_balance_ratio"] = convertir_a_float(min_class / max_class) if max_class > 0 else np.nan
    meta["majority_class_error"] = convertir_a_float(1.0 - (max_class / len(y))) if len(y) > 0 else np.nan
    ...
    bloques = [
        ...
        (["best_node", "random_node", "linear_discr", "elite_nn", "naive_bayes", "one_nn", "worst_node"],
         ["mean"],
         ["best_node_mean", "random_node_mean", "linear_discr_mean", "elite_nn_mean",
          "naive_bayes_mean", "one_nn_mean", "worst_node_mean"]),
    ...
    for features, summary, keys in bloques:
        try:
            bloque = _extraer_bloque(X_df, y, features=features, summary=summary)
            for key in keys:
                meta[key] = bloque.get(key, np.nan)
        except Exception:
            for key in keys:
    ...
    try:
        bloque = _extraer_bloque(X_df, y, features=["f1", "n2", "n3", "n4"], summary=["mean"])
